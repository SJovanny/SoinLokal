create table if not exists nurse_care_types (
...
drop policy if exists "Nurses can view their own care types" on nurse_care_types;
create policy "Nurses can view their own care types"
  on nurse_care_types for select
  using (auth.uid() = nurse_id);

drop policy if exists "Nurses can insert their own care types" on nurse_care_types;
create policy "Nurses can insert their own care types"
  on nurse_care_types for insert
  with check (auth.uid() = nurse_id);

drop policy if exists "Nurses can delete their own care types" on nurse_care_types;
